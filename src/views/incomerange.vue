...
export default {
  ...
  data: () => ({
    isloading: false,
    loadingText: 'Loading Expenditure by IncomeRange',
    labels: [],
    data: [],
    ChartMsg: 'No Data',
    title: 'Expenditure by IncomeRange'
  }),
  mounted () {
      this.getSpendData()
  },
  computed: {
    ...
    chartConfig () {
      return {
        ...
        title: {
          text: 'Expenditure by IncomeRange'
        },
          tooltip: {
  ...
  methods: {
    async getSpendData () {
      ...
      await Axios.get('https://ccmidterm1.azurewebsites.net/backend/getincomechart/').then((data)=>{
